...
module.exports = {
    ...
    exitsHead: function (text) {
        ...
        return text.trim().length > 0 && TAGS_WITH_TEXT_ALLOWED_IN_HEAD.indexOf(this.ctx.leadingStartTag) < 0;
    },

    onStartTag: function (startTag) {
        if (!this.ctx.inBody) {
            this.ctx.inBody = TAGS_ALLOWED_IN_HEAD.indexOf(startTag.tagName) < 0;
